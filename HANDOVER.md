...
| 3 | **Warm G4, copy and zero-extension debris**: sound frontend MOV/MVN/NEG alias folds, MADD/MSUB (MUL/MNEG) and MultiplyAddSubLong (SMULL/UMULL) alias folds with native `_SMull`/`_UMull` lowering in `TranslateDataProcessing.cpp`, and commutative constant identities in `ALUOps.cpp` landed. Producer masking and 64-bit consumer reads kept soundly isolated without touching RA or eliding zero-extensions. Warm slice 17.37 s -> 17.21 s; zlib 46.6 s -> 36.3 s, Lua 41.3 s -> 32.1 s | warm §7 G4 | frontend folds & commutative identities done | complete |
