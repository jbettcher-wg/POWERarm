...
| N6 | One-instruction gap fills: `SQDMULH/SQRDMULH.8H`=`vmhaddshs/vmhraddshs`; `UDOT`=`vmsumubm`; `PMULL`=`xxmrgld+vpmsumd`; `UABD`=`vabsdu*` (P9); `UQADD/UQSUB.2D` in 3; `SQADD.2D` in 9 | NEON §3.4–3.6, §3.14–3.15 | SIMD translator/lowering | new coverage at native-like cost | done | Coverage landed: UDOT = one vmsumubm (new VUDot op); PMULL.2D via PCLMUL (xxpermdi + vpmsumd), PMULL.8B via new VPMullB (vmrgl/h with zero + vpmsumb); UQADD/UQSUB.2D in 3 ops, SQADD/SQSUB.2D in 7-8; UABD/UABDL via new VUABD op using ISA 3.0 vabsdu* (3→1 insn, 5.7→3.0 cyc on P9; P8 fallback); SQDMULH/SQRDMULH.8H lowered via new VSQDMulH/VSQRDMulH ops to native vmhaddshs/vmhraddshs (1 insn + zero, ~25→2 insns); 32-bit forms via widening multiply fallback. Gate 1: 83 pass, 0 fail (default + disableisa30); Gate 2: 24/24 pass; Gate 3: 0 required fail. Parity: simd_sat, simd_crypto. |
