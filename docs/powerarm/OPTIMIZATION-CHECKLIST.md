...
| N9 | AES rounds: `vxor;vcipher` with the state kept byte-reversed; AESE/AESD/AESMC/AESIMC 3–6 instructions; fix the `vsldoi 4`/`REV32` emitter comment | NEON §3.15, §4.2 | crypto ops | exact AES at native cost | done | Lowered directly via dedicated VAESE (3 insns: vxor + vcipherlast + vsldoi 4), VAESD (3 insns: vxor + vncipherlast + vsldoi 12), VAESMC (4 insns on ISA 3.0: xxbrw + vncipherlast + vcipher + xxbrw; 6 on ISA 2.07), and VAESImc (4 insns on ISA 3.0: xxbrw + vcipherlast + vncipher + xxbrw; 6 on ISA 2.07). Replaces multi-IR-op synthesis and quadword byte-reversal roundtrips. Fixed AES comment in Emitter.h documenting LE vs BE lane layout, vsldoi rotation, and REV32 conjugation. Gate 1 passes 86/86 in default, disableisa30, and MAXINST=1 modes. |
...
| N13 | VSX-aware allocator class: pin V16–V31 into vs0–vs31 for float-only/`xv*` values and VSX-only temporaries (not per-op moves; context round trip 15.2 cycles, half-to-half 4) | NEON §2(a); FP §8, §13 | vector register allocator | removes the ~18% of cycles lost to V16–V31 context traffic in FP blocks | done | Pinned guest V16–V31 into host vs16–vs31 (`NumStaticVectorRegs = 32`). LoadV/StoreV emit `_LoadRegister`/`_StoreRegister` instead of context memory spills; `DEF_OP(LoadRegister)`/`DEF_OP(StoreRegister)` use `GetVSXReg` and `xxlor` (2 cycles vs 15.2-cycle context store-forwarding stall). RAPass updated for 32 registers without 1<<32 UB; low-bank V16–V31 bypass SRA coalescing so VMX instructions safely use dynamic RAFPR (v16–v29). ScalarSplatChain extended to 32 vector registers. SpillStaticRegs/FillStaticRegs save/restore vs16–vs31 with ISA 3.0 stxvx/lxvx and ISA 2.07 stxvd2x/lxvd2x+xxpermdi fallbacks. SignalDelegator saves low-bank vs16–vs31 via GetPPCVSXLowBankDW0/DW1. Gate 1 passes 100% in default, disableisa30, and MAXINST=1 modes. |
...
| C16 | Cache size cap (`CodeCacheMaxSize`, 8192 MiB), eviction of whole namespaces -- another build's first, then LRU -- other builds' namespaces removed after 1 h unused | CODE-CACHE.md "Default-on" | `Core/CodeCache.cpp`, `Config.json.in` | default-on | done | sweep after a published segment, at most once a minute; 20 MiB cap test: 187 -> 7.5 MiB, stale foreign namespaces removed; check-code-cache.sh passes (`evict` case) |
...
| C21 | Save publishing off the guest thread: the periodic and unmap passes hand the namespace lock, the link, the compaction, the write-back and the sweep to a writer process (`CodeCacheForkWriter`), which waits for the lock instead of dropping the segment and its records | COLD-ROUND2 R1 | `Core/CodeCache.cpp`, `Config.json.in`, `FEXInterpreter.cpp` | default-on | done | check-code-cache.sh `lockbusy`; contend. First shipped as a `fork(2)` of the running guest, which could deadlock inside `fork(2)` and copied a multi-GB guest's page tables every pass; now `posix_spawn` of this binary with `--codecache-writer` |
| C22 | Two tiers: the working cache in RAM (`CodeCacheHotTier`, `$XDG_RUNTIME_DIR/powerarm/cache`, `CodeCacheHotMaxSize`) over the durable copy on disk; seeded lazily per namespace, written back by the writer process, validated before each copy and renamed into place | owner direction | `Core/CodeCache.cpp`, `Config.json.in` | default-on | done | check-code-cache.sh `reseed`, `torn`, `evict` (hot cap); no timing taken |
