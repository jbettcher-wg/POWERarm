...
| C21 | Save writing off the guest thread: the periodic and unmap passes fork a writer (`CodeCacheForkWriter`), which waits for the namespace lock instead of dropping the segment and its records | COLD-ROUND2 R1 | `Core/CodeCache.cpp`, `Config.json.in` | default-on | done | check-code-cache.sh `lockbusy`; contend; item 21(a) (compaction SIGSEGV in a forked child) did not reproduce |
| C22 | Two tiers: the working cache in RAM (`CodeCacheHotTier`, `$XDG_RUNTIME_DIR/powerarm/cache`, `CodeCacheHotMaxSize`) over the durable copy on disk; seeded lazily per namespace, written back by the forked writer, validated before each copy and renamed into place | owner direction | `Core/CodeCache.cpp`, `Config.json.in` | default-on | done | check-code-cache.sh `reseed`, `torn`, `evict` (hot cap); no timing taken |
