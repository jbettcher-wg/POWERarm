...
| N10 | `LD2-4`/`ST2-4` interleave | NEON §3.13 | SIMD load/store | coverage + speed | done fe54a952c | LD2/ST2 = one unzip/zip per register (1 instruction for 8-32-bit lanes); LD3/LD4/ST3/ST4 = two VTBL2 lookups with constant indices plus VOr per register (4+4+1 on ISA 3.0 after N8). Single-structure LD1-4/ST1-4 and LD1R-4R added. Speed not measured separately |
